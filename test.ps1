function Connect-Mstsc {
    ...
    Description 
    -----------     
    Retrieves the LAPS managed local administrator password for server01 as a securestring
    #>
        [CmdletBinding()]
        ...
        $Password = ([adsisearcher]"samaccountname=$($ComputerName)$").findone().GetDirectoryEntry().'ms-mcs-admpwd'
        if ($Password) {
            return ConvertTo-SecureString -String ([string]$Password) -AsPlainText -Force
        }
    }

    Connect-Mstsc -ComputerName LAPTOP-COHESION-ARWIN -User LAPTOP-COHESION-ARWIN\Arwin -Password (ConvertTo-SecureString 'test' -AsPlainText -Force)